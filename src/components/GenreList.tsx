        ...
        <List>
            {data.map(genre => 
                <HStack paddingY='5px' key={genre.id}>
                    ...
                    <Button fontWeight={genre.id === selectedGenre?.id ? 'bold' : 'normal'} color={genre.id === selectedGenre?.id ? 'green' : ''} fontSize='lg' variant='link' onClick={() => onSelectGenre(genre)}>{genre.name}</Button>
                </HStack>
                )}
        </List>
    </>
